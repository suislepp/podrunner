...
update_wheels.py   # refreshes wheels/ from requirements/ — run after changing deps
pod.yaml           # pod manifest template (edit to change image/resources)
README.md
wheels/            # committed .whl files (regenerated by update_wheels.py)
...
1. Edit (or create) the relevant file in `requirements/`. Use normal pip syntax — package names with optional version specs:
   ```
   psycopg[binary]
   pandas==2.2.0
   requests>=2.31
   ```
2. Run `python3 update_wheels.py` from the repo root. This wipes `wheels/` and re-downloads everything declared across all `requirements/*.txt` files, with the right platform / Python version flags so the wheels work inside the pod.
3. Commit both `wheels/` and `requirements/`.

`update_wheels.py` hardcodes `--platform manylinux2014_x86_64`, `--python-version 3.11`, and `--only-binary=:all:` — those flags ensure you get wheels (not source distributions) that match the pod's Linux x86_64 / Python 3.11 environment. Wrong platform or Python version → `pip install` fails in the pod with "no matching distribution".

`pip download` pulls transitive deps automatically, so you don't have to chase them.

If you change the image's Python version in `pod.yaml`, update `PYTHON_VERSION` at the top of `update_wheels.py` and re-run it.
...
If you change the Python version in the image, also update `PYTHON_VERSION` at the top of `update_wheels.py` and re-run it to regenerate `wheels/`.
